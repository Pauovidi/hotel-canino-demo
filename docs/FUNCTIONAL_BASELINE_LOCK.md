...
| Availability-first with pet | `hay hueco este finde para PIPO?` records range + pet and asks for contextual review without fake yes/no. | Current hotfix. | Locked | `keeps availability-first contextual when pet and weekend range are present` | Bot invents availability or asks duplicate fields. |
