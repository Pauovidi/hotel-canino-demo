...
| Availability continuation pet slot | After availability-first asks for pet name, `PAPO` is applied as `availabilityInquiry.petName`, a preliminary read-only calendar precheck runs when range+pet are present, and the bot still asks for entry/exit hours for precision. | Availability times/precheck hotfix. | Locked | `continues availability-first by applying a standalone pet slot...`, `runs a date-only calendar precheck...`, `locks availability-first continuation...` | User gets generic fallback, no calendar read, or fake final availability. |
| Availability time slot continuation | After range+pet, `a las 10` is kept as `approximateTime=10:00` and asks a specific target clarification instead of repeating generic copy; `a las 10 entrada y salida` applies both times and runs the checker read-only. | Availability times/precheck hotfix. | Locked | `handles the real availability flow with a single time...`, `applies an explicit shared time...` | Useful time slots are ignored and the user sees the same prompt again. |
| Availability-first with pet | `hay hueco este finde para PIPO?` records range + pet, runs preliminary calendar precheck read-only, and asks for entry/exit times without claiming a confirmed reservation. | Availability times/precheck hotfix. | Locked | `asks only for times when the read-only checker still lacks slots`, `smoke:functional-baseline` | Bot invents availability, skips calendar read, or asks duplicate form fields. |
...
| Availability calendar precheck read-only | With `dateStart + dateEnd + petName` but missing times, availability-first may call the same read path with conservative default slots and mark the result `available_preliminary`, never final. | Availability times/precheck hotfix. | Locked | `runs a date-only calendar precheck...` | Missing hours block all calendar visibility or preliminary copy is mistaken for a final booking. |
...
| No internal availability copy | Availability-first visible replies must not contain `flujo operativo`; use `calendario` or `calendario de reservas`. | Availability times/precheck hotfix. | Locked | `renders availability-first keys without fake availability`, `smoke:functional-baseline` | Internal implementation language leaks to WhatsApp. |
